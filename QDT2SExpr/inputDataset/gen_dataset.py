...
def cwq_read_gen_examples_from_json(
    ...
    if use_qdt:
        print("decomposition file name:", qdt_file)
        qdt_bank = load_json(qdt_file)
        if isinstance(qdt_bank, list):  # old qdt
            qdt_bank = {x['ID']: x['pred'] for x in qdt_bank}
        else:
            # new qdt
            pass
    ...
    for data in tqdm(split_bank, desc="Reading", total=len(split_bank)):
        ...
        ex = proc_cwq_gen_exs(data, qdt_data, cand_entities, cand_relations, is_eval, use_qdt, add_entity, add_relation,
                              gold_entity, gold_relation, gold_relation_num)
...
def proc_cwq_gen_exs(example: dict,
    ...
    # query = example["subq_1"]
    # query = example["subq_2"]
    ...
    if len(entity_label_map) == 0:
        ...
        linear_origin_map = {l: e for e, l in entity_label_map.items()}.update(
            {l: r for r, l in relation_label_map.items()})
...
def _extract_gen_feature_from_example(args, tokenizer, ex,
    ...
    # print("seperator: decomposition structure:")
        # 用不用qdt****************
    if args.add_entity:
        if args.gold_entity:
    ...
    src_text = src_text.lower()  # all do lower
    dst_text = gt_lf.lower()  # all do lower
...
def cwq_load_and_cache_gen_examples(args, tokenizer, evaluate=False) -> ListDataset:
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if args.new_qdt:
            # new_code_42_1203_tree
            qdt_file = '../../Clue-Decipher/data/aaai_out/cwq_qa_decomp_seq2seq_42_1203_' + split_id + '.json'
        ...
        candidate_entity_file = os.path.join(
            ...
            # input_dir, "linking_results", f"merged_WebQSP_{split_id}_linking_results.json"
        )
