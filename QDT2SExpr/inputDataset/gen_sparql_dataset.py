...
def _vanilla_linearization_method(expr, entity_label_map, relation_label_map, linear_origin_map):
    ...
    for t in toks:
        ...
        if t.startswith("m."): # replace entity with its name
...
def _vanilla_linearization_sparql_method(sparql, entity_label_map, relation_label_map, linear_origin_map):
    # print(sparql)
    lines = sparql.split('\n')
    ...
    for line in lines:
        ...
        for tok in tokens:
            if tok.startswith('?'): # variable
                ...
                new_tok = tok
                linear_origin_map[new_tok] = tok
            elif tok.startswith('ns:m.'):
                new_tok = tok.replace('ns:','')
                label = get_label_with_odbc(new_tok)
                new_tok = "[ " + label + " ]"
                linear_origin_map[new_tok] = tok
                entity_label_map[new_tok] = label
            elif tok.startswith('ns:'): # relation
                new_tok = tok.replace('ns:','')
                new_tok = new_tok.replace('.',' , ')
                new_tok = new_tok.replace('_',' ')
                new_tok = "[ "+ new_tok+" ]"
                linear_origin_map[new_tok] = tok
                relation_label_map[tok] = new_tok
            elif tok=='.':
                new_tok = '.'
            elif tok=="<":
                new_tok = tok.replace("<",'[lt]')
            elif tok==">":
                new_tok = tok.replace(">",'[gt]')
            elif tok=="<=":
                new_tok = tok.replace("<=","[le]")
            elif tok==">=":
                new_tok = tok.replace(">=","[ge]")
            else:                
                new_tok = tok
                if "^^" in tok:
                    new_tok = tok.replace("^^","#")

            norm_toks.append(new_tok)

        norm_lines.append(' '.join(norm_toks))         
    
    normed_sparql = ' '.join(norm_lines)
    # print(normed_sparql)
    return normed_sparql
...
def cwq_read_gen_sparql_examples_from_json(
    ...
    for data in tqdm(split_bank, desc="Reading", total=len(split_bank)):
        ...
        if add_relation:
            ...
            if not candidate_relation_logit_list:
                ...
                if candidate_relation_num>0:
                    ...
                    if candidate_relation_num==1:
                        # top 1 candidate relation
                        cand_relations = [x[0] for x in candidate_relation_logit_list[:1] if x[1]>0]
                    else:    
                        cand_relations = [x[0] for x in candidate_relation_logit_list[:candidate_relation_num] if x[1]>0]
                else:
        ...
        ex = proc_cwq_gen_sparql_exs(data, qdt_data, cand_entities, cand_relations, is_eval, use_qdt, add_entity, add_relation, gold_entity, gold_relation, gold_relation_num)
...
def proc_cwq_gen_sparql_exs(example:dict, 
    ...
    sparql = example["sparql"] # ground truth sparql
    ...
    norm_sparql = _vanilla_linearization_sparql_method(sparql, entity_label_map, relation_label_map, linear_origin_map)
    ...
    gt = LFCandidate(gt_expr, norm_sparql, True, 1.0, 0.0)
...
def cwq_load_and_cache_gen_sparql_examples(args, tokenizer, evaluate=False)-> ListDataset:
    ...
    cachefile_name = "gen_sparql_{}_{}_{}".format(dataset_id, split_id, args.model_type, args.use_qdt)
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        examples = cwq_read_gen_sparql_examples_from_json(dataset_file, 
                                                split_file, 
                                                qdt_file, 
