from components.disamb_dataset import GrailEntityDisambFeature, extract_disamb_features_from_examples
...
class _MODULE_DEFAULT:
    ...
    RELATION_FREQ_FILE = 'data/relation_freq.json'
    RELATION_FREQ = None
...
def load_and_cache_disamb_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    cached_features_file = os.path.join(
        'feature_cache',"disamb_{}_{}_{}_{}".format(dataset_id, split_id,args.model_type,args.max_seq_length)
    )
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        example_cache = os.path.join('feature_cache', f'{dataset_id}_{split_id}_disamb_example.bin')
        if os.path.exists(example_cache) and not args.overwrite_cache:
            ...
            if dataset_id.lower()=='cwq':
                ...
                dataset_file = os.path.join('data','WebQSP', 'origin',f'WebQSP.{orig_split}.json')
                examples = read_disamb_instances_from_entity_candidates(dataset_file, candidate_file)
