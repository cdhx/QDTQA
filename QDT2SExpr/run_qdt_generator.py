...
from inputDataset.gen_qdt_dataset import cwq_load_and_cache_qdt_examples
from components.generation_trainer import GenerationTrainer
from components.utils import dump_json
from torch.utils.data import DataLoader, SequentialSampler

from inputDataset.gen_dataset import ListDataset, generation_collate_fn
...
def run_prediction(args, dataset, model, tokenizer, output_prediction=False):
    ...
    def qdt_exact_match(pred_qdt,gold_qdt):
        pred_qdt = pred_qdt.strip()
        gold_qdt = gold_qdt.strip()
        if pred_qdt.lower()==gold_qdt.lower():
            return True
        return pred_qdt.lower()==gold_qdt.replace('  ',' ').lower()
    ...
    for feat, pred in tqdm(zip(dataset, all_predictions), total=len(all_predictions),desc="Decoding"):
        ...
        # ex_cnt +=(decoded_pred[0].lower() == ex.qdt.strip().lower())
        ex_match = qdt_exact_match(decoded_pred[0],ex.qdt)
        contains_exact_match = any([qdt_exact_match(pred,ex.qdt) for pred in decoded_pred])
        ex_cnt += ex_match
        contains_ex_cnt += contains_exact_match

        if ex.qdt!='' and ex.qdt.lower()!='null':
            total+=1
        ...
        predict_data['question'] = ex.question
        ...
        predict_data['ex_match'] = ex_match
        predict_data['contains_ex'] = contains_exact_match
    ...
    return  {
            ...
            'contains_ex': contains_ex_cnt/total
            }
...
def main():
    ...
    if training_args.do_train:
        ...
        train_dataset = ListDataset(cwq_load_and_cache_qdt_examples(model_args, tokenizer, evaluate=False))
    else:
        train_dataset = ListDataset([])

    if training_args.do_eval:
        # eval_dataset = ListDataset(load_and_cache_examples(model_args, tokenizer, evaluate=True))
        eval_dataset = ListDataset(cwq_load_and_cache_qdt_examples(model_args, tokenizer, evaluate=True))
        # pass
    else:
