...
def aggressive_top_k_eval_new(split, predict_file, dataset):
    ...
    if dataset == "CWQ":
        # train_gen_dataset = load_json(f'data/CWQ/merged_all_data/2hopValidation_maskMention_richRelation_CrossEntropyLoss_top100_candidate_entities_merged_FACC1_elq/CWQ_train_all_data_entity_relations.json')
        # test_gen_dataset = load_json(f'data/CWQ/merged_all_data/2hopValidation_maskMention_richRelation_CrossEntropyLoss_top100_candidate_entities_merged_FACC1_elq/CWQ_test_all_data_entity_relations.json')
        # dev_gen_dataset = load_json(f'data/CWQ/merged_all_data/2hopValidation_maskMention_richRelation_CrossEntropyLoss_top100_candidate_entities_merged_FACC1_elq/CWQ_dev_all_data_entity_relations.json')
        train_gen_dataset = load_json(f'data/CWQ/CWQ_train_expr.json')
        test_gen_dataset = load_json(f'data/CWQ/CWQ_test_expr.json')
        dev_gen_dataset = load_json(f'data/CWQ/CWQ_dev_expr.json')
    elif dataset == "WebQSP":
        # train_gen_dataset = load_json(f'data/WebQSP/merged_all_data/richRelation_2hopValidation_richEntity_CrossEntropyLoss_top100_1parse_ep6/WebQSP_train_all_data_noQdt.json')
        # test_gen_dataset = load_json(f'data/WebQSP/merged_all_data/richRelation_2hopValidation_richEntity_CrossEntropyLoss_top100_1parse_ep6/WebQSP_test_all_data_noQdt.json')
        # train_gen_dataset = load_json(f'data/WebQSP/merged_all_data/richRelation_2hopValidation_richEntity_CrossEntropyLoss_top100_1parse_ep6/WebQSP_train_all_data_noQdt_top20CandRelations.json')
        # test_gen_dataset = load_json(f'data/WebQSP/merged_all_data/richRelation_2hopValidation_richEntity_CrossEntropyLoss_top100_1parse_ep6/WebQSP_test_all_data_noQdt_top20CandRelations.json')
        train_gen_dataset = load_json(f'data/WebQSP/structure_filling/richRelation_2hopValidation_richEntity_CrossEntropyLoss_top100_1parse_ep6/FACC1_elq_entities/WebQSP_train_all_data_sexpr_masked_candidate_structures.json')
        test_gen_dataset = load_json(f'data/WebQSP/structure_filling/richRelation_2hopValidation_richEntity_CrossEntropyLoss_top100_1parse_ep6/FACC1_elq_entities/WebQSP_test_all_data_sexpr_masked_candidate_structures.json')
        # dev_gen_dataset = load_json(f'data/WebQSP/merged_all_data/richRelation_2hopValidation_richEntity_CrossEntropyLoss_top100_1parse_ep6/WebQSP_test_all_data_noQdt.json')
    ...
    if dataset == "CWQ":
        gold_label_maps = load_json(f"data/CWQ/label_maps/CWQ_{split}_label_maps.json")    
        train_entity_map = load_json(f"data/CWQ/label_maps/CWQ_train_entity_label_map.json")
        train_entity_map = {l.lower():e for e,l in train_entity_map.items()}
    elif dataset == "WebQSP":
        gold_label_maps = load_json(f"data/WebQSP/label_maps/1parse_ep6/WebQSP_{split}_label_maps.json")
        train_entity_map = load_json(f"data/WebQSP/label_maps/1parse_ep6/WebQSP_train_entity_label_map.json")
        train_entity_map = {l.lower():e for e,l in train_entity_map.items()}

    if not use_goldEnt:
        if dataset == "CWQ":
            # candidate_entity_map = load_json(f"exps/gen_multitask/CWQ_no_qdt_not_lower_no_entity_relation_entity_multitask_disambigution_10epoch/CWQ_test_predicted_entity_linking_results.json")
            candidate_entity_map = load_json(os.path.join(dirname, 'CWQ_test_predicted_entity_linking_results_sorted.json'))
            # candidate_entity_map = ('exps/gen_multitask/CWQ_concat_relation_normalized_concat_entity_relation_first_not_lower_no_qdt_no_eval_15epoch/CWQ_test_predicted_entity_linking_results_sorted.json')
            # candidate_entity_map = load_json(f"data/CWQ/linking_results/merged_CWQ_{split}_linking_results.json")
            # candidate_entity_map = load_json(f"data/CWQ/entity_linking_0414/CWQ_{split}_merged_elq_FACC1.json")
            # candidate_entity_map = load_json(f"exps/gen_multitask/CWQ_test_predicted_entity_linking_results_sorted.json")
            train_type_map = load_json(f"data/CWQ/label_maps/CWQ_train_type_label_map.json")
            train_type_map = {l.lower():t for t,l in train_type_map.items()}
        elif dataset == "WebQSP":
            candidate_entity_map = load_json(os.path.join(dirname, "WebQSP_test_predicted_entity_linking_results_sorted.json"))
            # candidate_entity_map = load_json("exps/gen_multitask/WebQSP_concat_relation_normalized_concat_entity_relation_first_not_lower_no_qdt_15epochs/WebQSP_test_predicted_entity_linking_results_sorted.json")
            # candidate_entity_map = load_json(f"data/WebQSP/linking_results/no_dev/WebQSP_{split}_linking_results_rng.json")
            train_type_map = load_json(f"data/WebQSP/label_maps/1parse_ep6/WebQSP_train_type_label_map.json")
            train_type_map = {l.lower():t for t,l in train_type_map.items()}
        
    if not use_goldRel:
        if dataset == "CWQ":
            train_relation_map = load_json(f"data/CWQ/label_maps/CWQ_train_relation_label_map.json")
            train_relation_map = {l.lower():r for r,l in train_relation_map.items()}
        elif dataset == "WebQSP":
            train_relation_map = load_json(f"data/WebQSP/label_maps/1parse_ep6/WebQSP_train_relation_label_map.json")
            train_relation_map = {l.lower():r for r,l in train_relation_map.items()}
    ...
    for (pred,gen_feat) in tqdm(zip(predictions,gen_dataset), total=len(gen_dataset), desc=f'Evaluating {split}'):
        ...
        if not use_goldEnt:
            ...
            if qid in candidate_entity_map:
                ...
                # entity_label_map = {}
                # for e,l in candidate_entity_map[qid].items():
                #     if l['label'].lower() not in entity_label_map: # 链接到有相同 label 的实体，以排序最靠前的实体为准
                #         entity_label_map[l['label'].lower()] = e
            else:
                entity_label_map = {}
            # entity_label_map = {item['label'].lower(): item['id'] for item in candidate_entity_map[qid]}
            # entity_label_map = {l['label'].lower():e for e,l in candidate_entity_map[qid].items()}
            
            # entity mention map, Dict[mention:mid]
            # entity_mention_map = {l['mention'].lower():e for e,l in candidate_entity_map[qid].items()}
            entity_mention_map = None
            # entity_mention_map = {item['mention'].lower(): item['id'] for item in candidate_entity_map[qid] if 'mention' in item}
        ...
        for rank, p in enumerate(pred['predictions']):
            ...
            if dataset == "CWQ" and rank == 0 and lf.lower() ==gen_feat['SExpr'].lower():
                ex_cnt +=1
            elif dataset == "WebQSP" and rank == 0 and lf.lower() ==gen_feat['sexpr'].lower():
                ex_cnt +=1
