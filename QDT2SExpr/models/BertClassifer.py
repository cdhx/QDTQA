...
import torch
from torch import logit, nn, sigmoid
...
class BertForClassification(BertPreTrainedModel):
    """Use Bert to rank candidates, inheritated from BertPreTrainedModel"""
    ...
    def forward(
        ...
        if labels is not None:
            # input_ids.size: #[batch_size, sample_size, max_length]
            batch_size = input_ids.size(0)
            sample_size = input_ids.size(1) # default 100
        ...
        logits = self.classifier(pooled_output) #[batch_size*sample_size, 1]
    
        loss = None
        if labels is not None:
            
            labels = labels.float()  #[batch_size, sample_size]
            labels = labels.view(-1)
            # labels = labels.squeeze()

            # reshape logits
            logits = logits.view((batch_size, sample_size)) #[batch_size, sample_size]
