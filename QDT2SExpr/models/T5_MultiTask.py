...
class ExpLinear(nn.Module):
    ...
    def forward(self, input):
        # exp() assure weight all always positive (for different loss)
        print('weights: {}'.format(self.weights))
        return nn.functional.linear(input, torch.abs(self.weights), self.bias)
...
class T5_MultiTask_Relation(nn.Module):
    ...
    不拼接关系分类的预测结果
    """
    def __init__(self, pretrained_model_path, is_test=False, linear_loss=False, cross_entropy_loss=False) -> None:
        super().__init__()
    ...
    def forward(self,
        ...
        if clf_labels is not None:
            ...
            if self.cross_entropy_loss:
                ...
                print('L123 clf_loss: {}'.format(clf_loss))
                print('L124 clf_predict_logits: {}'.format(clf_predict_logits.shape))
                print('L125 clf_labels: {}'.format(clf_labels.shape))
            else:
        ...
        if clf_loss is not None:
            if self.linear_loss:
                linear_layer = ExpLinear(2, 1)
                total_loss = linear_layer(torch.Tensor((gen_loss, clf_loss)))
            else:
    ...
    def inference(self,
        ...
        with torch.no_grad():
            ...
            if self.cross_entropy_loss:
                ...
                print('L192: {}'.format(clf_outputs.shape))

        return gen_outputs,clf_outputs


class T5_MultiTask_Entity_Relation(nn.Module):
    """
    训练任务：实体分类 + 关系分类 + 生成
    不拼接实体分类、关系分类的预测结果
    """
    ...
    def forward(self,
        ...
        if entity_clf_loss is not None and relation_clf_loss is not None:
            if self.linear_loss:
                linear_layer = ExpLinear(3, 1)
                total_loss = linear_layer(torch.Tensor((gen_loss, entity_clf_loss, relation_clf_loss)))
            else:
    ...
    def inference(self,
        ...
        with torch.no_grad():
            ...
            # [batch_size*sample_size, 1] if BCELoss, [batch_size*sample_size, 2] if CrossEntropy
            relation_clf_predict_logits = self.cls_layer(self.dropout(relation_sentence_embedding))
            relation_clf_outputs = torch.reshape(relation_clf_predict_logits, (input_ids_gen.size(0),clf_sample_size,-1))
            if self.cross_entropy_loss:
...
class T5_MultiTask_Relation_Entity(nn.Module):
    ...
    """
    def __init__(self, pretrained_model_path, device='cuda', max_src_len=128, tokenizer=None, is_test=False, sample_size=10, do_lower=False, linear_loss=False, cross_entropy_loss=False) -> None:
        super().__init__()
        self._is_test = is_test
        self.t5 = T5ForConditionalGeneration.from_pretrained(pretrained_model_path)
        self.device = device
...
class T5_MultiTask_Concat_Relation(nn.Module):
    """ 
    训练任务：关系分类 + 生成
    ...
    def forward(self,
        ...
        if clf_labels is not None:
            ...
            if self.cross_entropy_loss:
                clf_loss = self.criterion(clf_predict_logits.float(), clf_labels)
                print('L418 clf_loss: {}'.format(clf_loss))
                print('L419 clf_predict_logits: {}'.format(clf_predict_logits.shape))
                print('L420 clf_labels: {}'.format(clf_labels.shape))
            else:
                 clf_loss = self.criterion(clf_predict_logits.float(), clf_labels.unsqueeze(1).float())
...
class T5_MultiTask_Concat_Entity_Concat_Relation(nn.Module):
    """ 
    训练任务：实体分类 + 关系分类 + 生成
    关系分类任务的输入，拼接实体分类的结果
    生成模型的输入，拼接实体分类和关系分类的结果
    """
    def __init__(self, pretrained_model_path, device='cuda', max_src_len=128, tokenizer=None, is_test=False, sample_size=10, do_lower=False, linear_loss=False, cross_entropy_loss=False) -> None:
        super().__init__()
        self._is_test = is_test
        ...
        self.do_lower = do_lower
        self.linear_loss = linear_loss # 是否用一个线性层来为不同的 loss 赋权值
    ...
    def forward(self,
        ...
        textual_input_src_gen=None,
        normalize_relations=False,
        textual_candidate_entities_list=None
        ):
        # clf_encoder_outputs: [batch_size, max_len, hidden_size]
        textual_candidate_relations = list_reshape(textual_candidate_relations, int(len(textual_candidate_relations)/self.sample_size), self.sample_size)
        textual_candidate_entities_list = list_reshape(textual_candidate_entities_list, int(len(textual_candidate_entities_list) / self.sample_size), self.sample_size) # list of [batch_size, sample_size]
        ...
        if entity_clf_labels is not None:
            ...
            if self.cross_entropy_loss:
                entity_clf_loss = self.criterion(entity_clf_predict_logits.float(), entity_clf_labels)
                print('L418 clf_loss: {}'.format(entity_clf_loss))
                print('L419 clf_predict_logits: {}'.format(entity_clf_predict_logits.shape))
                print('L420 clf_labels: {}'.format(entity_clf_labels.shape))
            else:
                entity_clf_loss = self.criterion(entity_clf_predict_logits.float(), entity_clf_labels.unsqueeze(1).float())
            ...
            filtered_candidate_entities = filter_candidates_by_classification_results(
                textual_candidate_entities_list,
        ...
        input_ids_relation_concatenated = []
        for (input_src, cand_ents, cand_relations_list) in zip(textual_input_src_gen, filtered_candidate_entities, textual_candidate_relations):
            for ent in cand_ents:
                input_src += " [ENT] " + ent
            if self.do_lower:
                input_src = input_src.lower()
            for cand_rel in cand_relations_list:
                tokenized_relation_src = self.tokenizer(
                    input_src,
                    cand_rel,
                    max_length=self.max_src_len,
                    truncation='longest_first',
                ...
                input_ids_relation_concatenated.append(tokenized_relation_src)
    
        # dynamic mini-batch padding
        relation_clf_encoded = self.tokenizer.pad({'input_ids': input_ids_relation_concatenated},return_tensors='pt')
        input_ids_relation_clf = relation_clf_encoded['input_ids'].to(self.device) # [batch_size*sample_size, max_len]
        relation_clf_attention_mask = relation_clf_encoded['attention_mask'].to(self.device) # [batch_size*sample_size, max_len]
        ...
        if relation_clf_labels is not None:
            ...
            if self.cross_entropy_loss:
                relation_clf_loss = self.criterion(relation_clf_predict_logits.float(), relation_clf_labels)
                print('L418 clf_loss: {}'.format(relation_clf_loss))
                print('L419 clf_predict_logits: {}'.format(relation_clf_predict_logits.shape))
                print('L420 clf_labels: {}'.format(relation_clf_labels.shape))
            else:
                relation_clf_loss = self.criterion(relation_clf_predict_logits.float(), relation_clf_labels.unsqueeze(1).float())
    ...
    def inference(self,
        ...
        # 1. Get prediction from Entity classification
        entity_clf_encoder_outputs = self.t5.encoder(input_ids=input_ids_entity_clf, attention_mask=entity_clf_attention_mask)
        entity_sentence_embedding = torch.mean(entity_clf_encoder_outputs.last_hidden_state,dim=1)
        # [batch_size*sample_size, 1] if BCELoss, [batch_size*sample_size, 2] if CrossEntropy
        entity_clf_predict_logits = self.cls_layer(self.dropout(entity_sentence_embedding))
        entity_clf_outputs = torch.reshape(entity_clf_predict_logits, (input_ids_gen.size(0),clf_sample_size,-1)) # [batch_size, sample_size, 1/2]
        ...
        # 2. Get prediction from relation classification (input should concat classified entities)
        input_ids_relation_concatenated = []
        for (input_src, cand_ents, cand_relations_list) in zip(textual_input_src_gen, filtered_candidate_entities, textual_candidate_relations):
            for ent in cand_ents:
                input_src += " [ENT] " + ent
        ...
        # [batch_size*sample_size, 1] if BCELoss, [batch_size*sample_size, 2] if CrossEntropy
        relation_clf_predict_logits = self.cls_layer(self.dropout(relation_sentence_embedding))
        relation_clf_outputs = torch.reshape(relation_clf_predict_logits,(input_ids_gen.size(0),clf_sample_size,-1)) # [batch_size, sample_size, 1]
