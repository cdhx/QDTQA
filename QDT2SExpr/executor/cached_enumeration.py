...
subclasses = defaultdict(lambda: set()) # subclass infos
...
for line in contents:
    ...
    if fields[2] in ['type.int', 'type.float']:
        numerical_relations.add(fields[1]) # numerical relations
    elif fields[2] == 'type.datetime':
        date_relations.add(fields[1]) # date relations
...
class FBLinkedRelationCache(FBCacheBase):
    ...
    def query_in_out_relation(self, entity):
        ...
        in_r, out_r = get_adjacent_relations_with_odbc(entity)
...
def legal_relation(r, dataset='grail', force_in_bank=True):
    ...
    if dataset == 'grail':
        if r.startswith('base.') or r.startswith('dataworld') or r.startswith('freebase'):
            return False
    if dataset == 'CWQ':
        if r.startswith('dataworld') or r.startswith('freebase'):
            return False
    return True
