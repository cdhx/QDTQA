...
def webqsp_evaluate_valid_results(args):
    """Compute P, R and F1 for WebQSP, only questions with executable queries count"""
    pred_data = load_json(args.pred_file)

    # origin dataset
    dataset_data = load_json(f'data/WebQSP/final/merged/WebQSP_{args.split}.json')
    ...
    with open (os.path.join(dirname,'final_eval_results.txt'),'w') as f:
        f.write(res)
        f.flush()
    ...
    dump_json(pred_data, os.path.join(dirname, f'{filename}_new.json'))
