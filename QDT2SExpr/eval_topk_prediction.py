...
from detect_and_link_entity import get_top1_entity_linking_from_earl
...
def denormalize_s_expr_new(normed_expr, entity_label_map,
                                        type_label_map,
                                        rel_label_map,
                                        train_entity_map,
                                        surface_index):
                        ...
                        if cur_seg.lower() in train_entity_map:
                            ...
                            if is_number(cur_seg):
                                ...
                                for ent_label in entity_label_map:
                                    string_sim = difflib.SequenceMatcher(None, cur_seg, ent_label).quick_ratio()
...
def aggressive_top_k_eval_new(split, predict_file):
    ...
    for (qid,pred) in tqdm(predictions.items(), total = len(predictions)):
        ...
        for rank, p in enumerate(pred):
            lf, answers = execute_normed_s_expr_from_label_maps(p, entity_label_map, 
            ...
            if rank == 0 and lf.lower() ==gen_feat['SExpr'].lower():
                ...
                if rank==0 and normed_sexpr_exmatch:
                    if not answers:
                        # normed ex_match, but not match after denormalization, and execute failed
                        denormalize_failed.append({
                            'ID':qid,
                            'gt_sexpr':gt_sexpr,
                            'gt_normed_sexpr':gt_normed_sexpr,
                            'denormed_sexpr':lf
                        })
    ...
    result_file = os.path.join(dirname,'gen_sexpr_results.json')
...
if __name__=='__main__':
    ...
    if args.qid:
        ...
        aggressive_top_k_eval_new(args.split, args.pred_file)
