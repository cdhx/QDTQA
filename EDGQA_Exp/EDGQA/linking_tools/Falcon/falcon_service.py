...
def sort_combinations(combinations, question):
    ...
    while i < len(questionWords):
        ...
        match = [s for s in combinations if any(
    ...
    while i < len(remainCombinations):
        ...
        if i+1 < len(remainCombinations):
            ...
            if check_only_stopwords_exist(question, current, remainCombinations[i+1], questionStopWords):
                temp = current+question[question.find(current)+len(current):question.rfind(
                    remainCombinations[i+1])]+remainCombinations[i+1]
                i = i+2
                continue
            else:
                if temp != "":
            ...
            if temp != "":
                if check_only_stopwords_exist(question, temp, remainCombinations[i], questionStopWords):
                    final = temp+question[question.find(temp)+len(temp):question.rfind(
                        remainCombinations[i])]+remainCombinations[i]
...
def reRank_results(relation, results):
    ...
    for result in results:
        distance = nltk.edit_distance(
            relation[relation.rfind('/')+1:], result[1][result[1].rfind('/')+1:])
...
def rank_entities_string_similarity(entities):
    for raw in entities:
        for entity in raw:
            if entity[2] != 0:
                entity[2] += (SequenceMatcher(None, entity[3],
                                              entity[1][entity[1].rfind('/')+1:]).ratio())*10
                entity[2] += (SequenceMatcher(None, entity[3],
                                              entity[0]).ratio())*20
    return entities
...
def relations_improvement_country(entities):
    ...
    for entity in entities:
        ...
        if results1['boolean']:
            relations.append(
                ["country", "http://dbpedia.org/ontology/country", 0, 20])
    return relations
...
def process_word_E_long(question):
    ...
    combinations = merge_comb_stop_words(
        combinations, question, questionStopWords)
    combinations = sort_combinations(combinations, question)
    combinations = merge_entity_prefix(
        question, combinations, originalQuestion)
...
def get_E_R(raw, k=1):
            ...
            if " and " in term:
                for word in term.split(' and '):
                    entityResults.extend(
                        searchIndex.entitySearch(word.strip()))
            if " or " in term:
    ...
    if (len(mixedRelations) == 0 and questionWord.lower() == "when") or compare_found:
        mixedRelations.append([["date", "http://dbpedia.org/ontology/date", 0, 20], [
                              "date", "http://dbpedia.org/property/date", 0, 20]])
        compare_found = False
    ...
    for i in range(len(mixedRelations)):
        ...
        mixedRelations[i], entities = reRank_relations(
            entities, mixedRelations[i], questionWord, questionRelationsNumber, question, k)
    ...
    if nationalityFlag:
        mixedRelations.append(
            ["country", "http://dbpedia.org/ontology/country", 20])
    ...
    # print("Entities:")
    # print(entities)
    entities = [[tup[1], tup[4]] for tup in entities]
    relations = [[tup[1], tup[4]] for tup in mixedRelations]
    ...
    # return [mixedRelations, entities]
    return result
...
def annotate():
    if not request.json or 'text' not in request.json:
        ...
        print("Process Question:"+request.json['text'])
        if 'k' not in request.json:
            raw = get_E_R([request.json['text']])
        else:
            k = request.json['k']
            raw = get_E_R([request.json['text']], int(k))
        print(raw)
        return jsonify(raw)
...
host = '0.0.0.0'
port = 9876
print('Falcon Server start at port:'+str(port))
