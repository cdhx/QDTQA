    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
            guid = "%s-%s" % (set_type, i)
            text_a = tokenization.convert_to_unicode(line[3])  # sentence1
            text_b = tokenization.convert_to_unicode(line[4])  # sentence2
            if set_type == "test":
                label = "0"  # set label=0 for test
            else:
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope("loss"):
        ...
        # loss = tf.losses.hinge_loss(one_hot_labels, logits, 1.0) # use hinge_loss
